...
#include "coverageonline_packet.h"
#include "coverageonline_packet_data.h"
#include <vector>

int hdr_coverageonline::offset_;

/*
 * CoverageOnline Header Class
 */
static class CoverageOnlineHeaderClass : public PacketHeaderClass {
public:
    CoverageOnlineHeaderClass() : PacketHeaderClass("PacketHeader/COVERAGEONLINE", sizeof(hdr_coverageonline)) {
        bind_offset(&hdr_coverageonline::offset_);
    }

    ~CoverageOnlineHeaderClass() { }
} class_coverageonlinehdr;

/*
 * CoverageOnline Agent Class
 */
static class CoverageOnlineAgentClass : public TclClass {
public:
    CoverageOnlineAgentClass() : TclClass("Agent/COVERAGEONLINE") { }

    TclObject *create(int, const char *const *) {
        return (new CoverageOnlineAgent());
    }
} class_coverageonline;

void
CoverageOnlineTimer::expire(Event *e) {
    ((CoverageOnlineAgent *) a_->*firing_)();
}

// ------------------------ Agent ------------------------ //
CoverageOnlineAgent::CoverageOnlineAgent() : GPSRAgent(),
                                             boundhole_timer_(this,
                                                              &CoverageOnlineAgent::holeBoundaryDetection) {
    hole_list_ = NULL;
...
int CoverageOnlineAgent::command(int argc, const char *const *argv) {
...
void CoverageOnlineAgent::recv(Packet *p, Handler *h) {
...
void CoverageOnlineAgent::recvCoverage(Packet *p) {
    struct hdr_ip *iph = HDR_IP(p);
    struct hdr_cmn *cmh = HDR_CMN(p);

    CoverageOnlinePacketData *data = (CoverageOnlinePacketData *) p->userdata();
...
/*----------------------------BOUNDARY DETECTION------------------------------------------------------*/
bool CoverageOnlineAgent::boundaryNodeDetection() {
...
void CoverageOnlineAgent::holeBoundaryDetection() {
    if (isBoundary) {
        Packet *p;
        hdr_cmn *cmh;
        hdr_ip *iph;
        hdr_coverageonline *ch;

        for (stuckangle *sa = cover_neighbors_; sa; sa = sa->next_) {
            p = allocpkt();

            CoverageOnlinePacketData *chpkt_data = new CoverageOnlinePacketData();
...
void CoverageOnlineAgent::addNeighbor(nsaddr_t addr, Point p) {
    GPSRAgent::addNeighbor(addr, p);

    addSensorNeighbor(addr, p, 2); // fixed add level 2 sensor neighbors
}

void CoverageOnlineAgent::addSensorNeighbor(nsaddr_t nid, Point location, int level) {
...
sensor_neighbor *CoverageOnlineAgent::getSensorNeighbor(nsaddr_t addr) {
    for (sensor_neighbor *temp = sensor_neighbor_list_; temp; temp = (sensor_neighbor *) temp->next_) {
...
node *CoverageOnlineAgent::getNextSensorNeighbor(nsaddr_t prev_node) {
...
bool CoverageOnlineAgent::isOutdatedCircle(node *intersect, triangle tri) {
    node *vertices = NULL;
    for (int i = 0; i < 3; i++) {
...
bool CoverageOnlineAgent::isSelectableTriangle(node *circle,
                                               node *intersect_a,
...
int CoverageOnlineAgent::black_node_count(int8_t **grid, int x, int y) {
    int count = 0;
