...
VR2OGraph::VR2OGraph(std::vector<CorePolygon> &cg_set_) {

    s_is_vertex_of_convex_ = false;
    d_is_vertex_of_convex_ = false;
    /*
     * add - special case
     */
    if (Geo::isVertexOfPolygon(cg_set_[0].vertices[0], cg_set_[1].vertices)) {
        s_is_vertex_of_convex_ = true;
        Point s = cg_set_[0].vertices[0];
        tagent_set_.push_back(Tangent(s, s, 0, 0));
        cg_set_.erase(cg_set_.begin());
    }

    Point d;

    if (Geo::isVertexOfPolygon(cg_set_[cg_set_.size() - 1].vertices[0], cg_set_[cg_set_.size() - 2].vertices)) {
        d_is_vertex_of_convex_ = true;
        d = cg_set_[cg_set_.size() - 1].vertices[0];
        cg_set_.pop_back();
    }
    ...
    if (d_is_vertex_of_convex_) {
        tagent_set_.push_back(Tangent(d, d, cg_set_.size() - 1, cg_set_.size() - 1));
    }
    ...
    for (unsigned int i = 0; i < tagent_set_.size() - 1; i++) {
...
double VR2OGraph::boundaryLength(Point a, Point b, CorePolygon cg) {
    ...
    length = length < polygonPerimeter(cg) - length ? length : polygonPerimeter(cg) - length;

    return length;
}

double VR2OGraph::boundaryLength(Point a, Point b, CorePolygon cg, bool &orient) {
    int a_ = getVertexIndexInPolygon(a, cg.vertices);
    int b_ = getVertexIndexInPolygon(b, cg.vertices);
...
int VR2OGraph::getVertexIndexInPolygon(Point t, std::vector<Point> p) {
...
double VR2OGraph::polygonPerimeter(CorePolygon cg) {
    double peri = 0;
    for (int i = 0; i < cg.vertices.size() - 1; i++) {
        int j = (i == cg.vertices.size() - 1) ? 0 : (i + 1);
        peri += G::distance(cg.vertices[i], cg.vertices[j]);
    }
    return peri;
}

std::vector<Point> VR2OGraph::boundaryPath(Point a, Point b, CorePolygon cg) {
    if (a == b)
...
void VR2OGraph::addVertexToPath(std::vector<int> parent, int j) {
    if (parent[j] != -1) {
...
double VR2OGraph::shortestPathLength() {
    findAllPaths();
...
std::vector<BasePathPoint> VR2OGraph::shortestPath() {
    findAllPaths();
    double min_length_ = DBL_MAX;
    int min_index_;
    ...
    return all_paths_points[min_index_];
}

std::vector<std::vector<BasePathPoint>> VR2OGraph::basePaths(double epsilon, int n) {
    findCuteAnglePaths();
    double length = 0;
   // double base_length = (1 + epsilon) * sin((n - 2) * M_PI / (2 * n)) * shortestPathLength();
    double base_length = shortestPathLength();
...
void VR2OGraph::findAllPaths() {

    if (s_is_vertex_of_convex_) {
        if (d_is_vertex_of_convex_) {
    ...
    for (int i = 0; i < all_paths_.size(); i++) {
        ...
        if (s_is_vertex_of_convex_) {
            single_path_points_.erase(single_path_points_.begin(), single_path_points_.begin() + 2);
        }
        if (d_is_vertex_of_convex_) {
            single_path_points_.pop_back();
        }
        if (!single_path_points_.empty())
            all_paths_points.push_back(single_path_points_);
    }
}

void VR2OGraph::addVertexToPath(int u, int d, bool *visited, int *path, int &path_index_) {
    visited[u] = true;
    path[path_index_] = u;
    path_index_++;
...
void VR2OGraph::findAllPaths(int s, int d) {
...
void VR2OGraph::findCuteAnglePaths() {
    findAllPaths();
    for (int i = 0; i < all_paths_points.size(); i++) {
...
bool VR2OGraph::isAllAcuteAngle(std::vector<BasePathPoint> path) {
    if (path.size() == 0)
        return true;
    Point s = path[0];
    ...
    for (int i = 0; i < path.size() - 1; i++) {
        if (G::abs_angle(path[i], path[i + 1], s, d) > M_PI / 2)
            return false;
    }
    return true;
}
