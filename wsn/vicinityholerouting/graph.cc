...
Graph::Graph(std::vector<Point> polygon) {
...
void Graph::addVertexToPath(std::vector<int> parent, int j) {
    if (parent[j] != -1) {
...
bool Graph::isVertexOfPolygon(Point s, std::vector<Point> p) {
    for (auto it : p) {
...
int Graph::getIndexInPolygon(Point s, std::vector<Point> p) {
...
std::vector<Point> Graph::shortestPath(Point s, Point t) {
