...
void ElbarGridOfflineAgent::recvData(Packet *p) {
    ...
    if(cmh->direction_ == hdr_cmn::UP && iph->daddr() == my_id_) {
        port_dmux_->recv(p,0);
        return;
    }
    else {
...
void ElbarGridOfflineAgent::sendData(Packet *p) {
    hdr_cmn*		cmh = HDR_CMN(p);
    hdr_ip*			iph = HDR_IP(p);
    hdr_elbar_grid* edh = HDR_ELBAR_GRID(p);

    cmh->size() += IP_HDR_LEN + edh->size();
    cmh->direction() = hdr_cmn::DOWN;

    edh->type_ = ELBAR_DATA;
    edh->forwarding_mode_ = GREEDY_MODE;
