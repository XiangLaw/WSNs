...
IGraph::IGraph(std::vector<Point> polygon) {
...
void IGraph::addVertexToPath(std::vector<int> parent, int j) {
    if (parent[j] != -1) {
...
bool IGraph::isVertexOfPolygon(Point s, std::vector<Point> p) {
    for (auto it : p) {
...
int IGraph::getIndexInPolygon(Point s, std::vector<Point> p) {
...
std::vector<Point> IGraph::shortestPath(Point s, Point t) {
