...
Angle G::abs_angle(Point p0, Point p1, Point p2, Point p3) {
    double re = atan2(p1.y_ - p0.y_, p1.x_ - p0.x_) - atan2(p3.y_ - p2.y_, p3.x_ - p2.x_);
    //  double atan_point1 = atan2(p1.y_ - p0.y_, p1.x_ - p0.x_);
    //   double atan_point2 = atan2(p3.y_ - p2.y_, p3.x_ - p2.x_);
    if (fabs(re) > M_PI)
        return 2*M_PI - fabs(re);
    else
        return fabs(re);
}
...
bool G::is_intersect2(Point *p1, Point *p2, Point *p3, Point *p4) {
    ...
    Line l2 = line(p3, p4);
    Point in;
    intersection(l1, l2, in);

    if (fabs(p1->x_ - p2->x_) < 0.0001)
        return (intersection(l1, l2, in)
                && ((in.y_ - p1->y_) * (in.y_ - p2->y_) < 0)
                && ((in.x_ - p3->x_) * (in.x_ - p4->x_) < 0)
                && ((in.y_ - p3->y_) * (in.y_ - p4->y_) < 0));

    if (fabs(p1->y_ - p2->y_) < 0.0001)
        return (intersection(l1, l2, in)
                && ((in.x_ - p1->x_) * (in.x_ - p2->x_) < 0)
                && ((in.x_ - p3->x_) * (in.x_ - p4->x_) < 0)
                && ((in.y_ - p3->y_) * (in.y_ - p4->y_) < 0));

    if (fabs(p3->x_ - p4->x_) < 0.0001)
        return (intersection(l1, l2, in)
                && ((in.x_ - p1->x_) * (in.x_ - p2->x_) < 0)
                && ((in.y_ - p1->y_) * (in.y_ - p2->y_) < 0)
                && ((in.y_ - p3->y_) * (in.y_ - p4->y_) < 0));

    if (fabs(p3->y_ - p4->y_ ) < 0.0001)
        return (intersection(l1, l2, in)
                && ((in.x_ - p1->x_) * (in.x_ - p2->x_) < 0)
                && ((in.y_ - p1->y_) * (in.y_ - p2->y_) < 0)
                && ((in.x_ - p3->x_) * (in.x_ - p4->x_) < 0));
...
bool G::is_intersect3(Point p1, Point p2, Point p3, Point p4) {
    ...
    Point in;
    bool a;
    a = (intersection(l1, l2, in))
        && (((in.x_ - p3.x_) * (in.x_ - p4.x_) < 0) || ((in.y_ - p3.y_) * (in.y_ - p4.y_) < 0));
    bool b = intersection(l1, l2, in);
    bool c = (in.x_ - p3.x_) * (in.x_ - p4.x_) < 0;
    bool d = (in.y_ - p3.y_) * (in.y_ - p4.y_) < 0;
    if (a == true)
        if ((fabs(in.x_ - p3.x_) < EPSILON && fabs(in.y_ - p3.y_) < EPSILON) ||
            (fabs(in.x_ - p4.x_) < EPSILON && fabs(in.y_ - p4.y_) < EPSILON))
            a = false;
    return a;
}
...
bool G::linePolygonIntersect(Point s, Point t, std::vector<Point> p) {
    int num_intersection = 0;
    if (s == Point(547, 1281) && t == Point(1018, 456))
        printf("here");
    for (unsigned int i = 0; i < p.size(); i++) {
        int j = (i == p.size() - 1 ? 0 : (i + 1));
