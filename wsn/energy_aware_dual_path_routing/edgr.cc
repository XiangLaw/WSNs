...
void EDGRAgent::recv(Packet *p, Handler *h) {
    ...
    switch (cmh->ptype()) {
        ...
        case PT_CBR:
            ...
            {
                if (cmh->num_forwards() == 0 && neighbor_list_ != NULL)   // a new packet
                {
            ...
            } else if (edgrh->dest != Point(0, 0)) {        // data packet
                recvData(p, edgrh);
            } else
...
void EDGRAgent::sendBurst(Packet *p) {
    if (this->my_id_ == 2569)
        printf("%d\there", Scheduler::instance().clock());
    struct hdr_cmn *cmh = HDR_CMN(p);
    struct hdr_ip *iph = HDR_IP(p);
                ...
                if (gdh->flag_ == 0) {
                    ...
                    if (nb_left == NULL) {
                        ...
                        if (dis_this >= dis_prev && dis_this >= dis_next &&
                            !isRedundantAnchor(gdh->anchor_list_, *this))
                        ...
                        if (dis_this >= dis_prev && dis_this >= dis_next &&
                            !isRedundantAnchor(gdh1->anchor_list_, *this))
                    ...
                    if (gdh->flag_ == EDGR_BURST_FLAG_LEFT) {
                        nb = getNeighborByLeftHandRule(gdh->dest_);
                    } else {
