...
Line::Line(Type type, const Point &start, const Point &end, const Vector &direction) : _type(type), _startPoint(start),
                                                                                       _endPoint(end),
...
LineIntersectionSolutionSet Line::intersection(const Line &line) const {
    ...
    if (!intersectionCoefficient(line, s) || !line.intersectionCoefficient(*this, t)) {
        return overlaps(line) ? LineIntersectionSolutionSet::infiniteSolutions()
...
LineIntersectionSolutionSet Line::lineIntersection(const Line &line) const {
    ...
    if (!intersectionCoefficient(line, s)) {
        return lineContains(line.supportVector()) ? LineIntersectionSolutionSet::infiniteSolutions()
                                                  : LineIntersectionSolutionSet::noSolution();
    }
