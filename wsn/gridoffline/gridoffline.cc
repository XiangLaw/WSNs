...
GridOfflineAgent::startUp()
{
	...
	fp = fopen("GridOffline.tr", 	"w");	fclose(fp);
...
GridOfflineAgent::createPolygonHole(Packet* p)
{
	...
	for (int i = 1; i <= data->size(); i++)
	{
		...
		{
			case Up:	y++;	if (maxy < y) maxy = y;	break;
			case Left:	x--;	if (minx > x) minx = x;	break;
			case Down:	y--;	if (miny > y) miny = y;	break;
			case Right:	x++;	if (maxx < x) maxx = x;	break;
		}
	}
...
GridOfflineAgent::dumpBoundhole()
{
	...
	for (polygonHole* p = hole_; p != NULL; p = p->next_)
	{
		node* n = p->node_list_;
		do {
			fprintf(fp, "%f	%f\n", n->x_, n->y_);
