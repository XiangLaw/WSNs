...
BEHDSAgent::recvBEHDS(Packet* p)
{
    ...
    // forward packet
    if (cmh->direction() == hdr_cmn::UP	&& edh->daddr == my_id_)	// up to original source
    {
        // add new circle hole
...
BEHDSAgent::recvData(Packet* p)
{
    ...
    {
        // meet hole
        if (circleHole_list_ && edh->saddr != my_id_ && edh->type_ == BEHDS_DATA_GREEDY)
        {
            sendHA(p, edh->saddr, edh->vertex[edh->vertex_num_]);
        }
