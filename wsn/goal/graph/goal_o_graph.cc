...
GoalOGraph::GoalOGraph(std::vector<CorePolygon> &cg_set_) {
...
double GoalOGraph::boundaryLength(Point a, Point b, CorePolygon cg) {
...
double GoalOGraph::boundaryLength(Point a, Point b, CorePolygon cg, bool &orient) {
    int a_ = getVertexIndexInPolygon(a, cg.vertices);
    int b_ = getVertexIndexInPolygon(b, cg.vertices);
...
int GoalOGraph::getVertexIndexInPolygon(Point t, std::vector<Point> p) {
...
double GoalOGraph::polygonPerimeter(CorePolygon cg) {
    double peri = 0;
...
std::vector<Point> GoalOGraph::boundaryPath(Point a, Point b, CorePolygon cg) {
    if (a == b)
...
void GoalOGraph::addVertexToPath(std::vector<int> parent, int j) {
    if (parent[j] != -1) {
...
double GoalOGraph::shortestPathLength() {
    findAllPaths();
...
std::vector<BasePathPoint> GoalOGraph::shortestPath() {
    findAllPaths();
    double min_length_ = DBL_MAX;
    int min_index_;
...
std::vector<std::vector<BasePathPoint>> GoalOGraph::basePaths(double epsilon, int n) {
    findCuteAnglePaths();
    double length = 0;
...
void GoalOGraph::addVertexToPath(int u, int d, bool *visited, int *path, int &path_index_) {
    visited[u] = true;
    path[path_index_] = u;
    path_index_++;
...
void GoalOGraph::findAllPaths(int s, int d) {
...
void GoalOGraph::findCuteAnglePaths() {
    findAllPaths();
    for (int i = 0; i < all_paths_points.size(); i++) {
...
bool GoalOGraph::isAllAcuteAngle(std::vector<BasePathPoint> path) {
    Point s = path[0];
